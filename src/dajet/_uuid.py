"""
_uuid.py — Конвертация UUID между форматами 1С и стандартным.
"""
...
def from_1c ( uuid_bytes: bytes ) -> UUID :
	"""
	1С → стандартный UUID.
	1С хранит первые 8 байт в обратном порядке.
	"""
	if len ( uuid_bytes ) != 16 :
		raise ValueError ( f"Expected 16 bytes, got {len ( uuid_bytes )}" )
	d = list ( uuid_bytes )
	return UUID (
		bytes = bytes ( [ d [ 3 ] , d [ 2 ] , d [ 1 ] , d [ 0 ] , d [ 5 ] , d [ 4 ] , d [ 7 ] , d [ 6 ] , *d [ 8 :16 ] ] ) )
